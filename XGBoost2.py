...
labels = train['y']
data = train.drop('galaxy', axis=1)
data = train.drop('y', axis=1)
correlation = abs(data.corrwith(labels))
columns = correlation.nlargest(80).index
...
train_data_train, train_data_valid, train_label_train, train_label_valid = train_test_split(train_data, train_labels,
                                                                                            train_size=0.8,
                                                                                            test_size=0.2,
                                                                                            random_state=0)
model = XGBRegressor(n_estimators=1000, learning_rate=0.05)
model.fit(train_data_train, train_label_train, early_stopping_rounds=5,
          eval_set=[(train_data_valid, train_label_valid)], verbose=False)

predictions = model.predict(test_data)
print(predictions)
test_pred = pd.DataFrame(0, index=np.arange(len(test)), columns=["predicted_y"])
test_pred.loc[0:, 'predicted_y'] = predictions
test["predicted_y"]=test_pred.to_numpy()
test.sort_index(inplace=True)
predictions = test["predicted_y"]
print(predictions)

pot_inc = -np.log(predictions + 0.01) + 3
p2 = pot_inc ** 2
# print(p2)
ss = pd.DataFrame({
    'Index': test.index,
...
ss[['Index', 'pred', 'opt_pred']].to_csv('submission_XGBoost2.csv', index=False)
